...
get_assoc(Key, Tree, Val) :-
    nonvar(Key),
    !,
    get_assoc1(Key, Tree, Val).
get_assoc(Key, Tree, Val) :-
    var(Key),
    !,
    get_assoc2(Key, Tree, Val).


get_assoc1(Key, t(K,Val,_,_), Val) :-
    Key == K,
    !.

get_assoc1(Key, t(K,_,L,_), Val) :-
    Key @< K,
    !,
    get_assoc1(Key,L,Val).

get_assoc1(Key, t(_,_,_,R), Val) :-
    !,
    get_assoc1(Key,R,Val).


get_assoc2(Key,t(_,_,L,_), Val) :-
    get_assoc2(Key,L,Val).

get_assoc2(Key,t(K,Val,_,_), Val) :- % Note: if you put t(Key,Val...) in the
                        % title line here, then var(Key) will
    Key = K.            % fail: the match instantiates it.

get_assoc2(Key,t(_,_,_,R),Val) :-
     get_assoc2(Key,R,Val).
